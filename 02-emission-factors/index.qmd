...
### Pool Chain Separations 

-   Problem: Reporting both biomass loss AND fire CO₂ emissions double-counts the same carbon
-   Rule: Fire CO₂ is captured in biomass stock change calculations

Safeguards:

-   When deforestation occurs with fire:

Recommended Example:

```         
1. Biomass Stock Change = B_final - B_initial (includes all CO₂)
2. Fire CH₄ Emissions = Area × Fuel Load × Combustion Factor × EF_CH₄ × GWP_28
3. Fire N₂O Emissions = Area × Fuel Load × Combustion Factor × EF_N₂O × GWP_265
```

Incorrect Example:

```         
Biomass Stock Change + Fire CO₂ Emissions
   ↑                        ↑
   └────────────────────────┴──── SAME CARBON COUNTED TWICE
```

::: callout-warning
## Critical Rule

ONLY report non-CO₂ gases (CH₄, N₂O) as separate fire emissions. CO₂ from biomass burning is already accounted for in the "Biomass Stock Change" calculation through the assumption of immediate oxidation (Tier 1 default).
:::

### 5.3 Regrowth Cycle Monitoring
...
Unless: Tier 2 data proves longer rotation (e.g., documented 30-year coffee systems)

### 5.4 Overlapping SOC Activities
...
General Principle: When uncertain about cumulative versus saturating effects, apply the most conservative interpretation and document rationale clearly.
