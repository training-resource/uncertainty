...
Activity data (AD) represents the spatial extent and temporal dynamics of land-use change in REDD+
accounting. This chapter addresses uncertainty quantification in land cover classification, change
detection, and spatial aggregation—typically the largest single contributor to total REDD+
uncertainty (40-60% of combined variance).

The chapter is structured around a data cube framework for organizing and analyzing spatiotemporal
land cover data, followed by uncertainty estimation techniques aligned with IPCC methodologies. We
organize the technical content into the following components:
...
easypackages::packages(
  "bslib", "cols4all", "covr", "cowplot", "dendextend", "digest","DiagrammeR","dtwclust", 
  "downlit", "e1071", "exactextractr","elevatr", "FNN", "future", "forestdata",
  "gdalcubes", "gdalUtilities", "geojsonsf", "geos", "ggplot2", "ggstats", 
  "ggspatial", "ggmap", "ggplotify", "ggpubr", "ggrepel", "giscoR", 
  "hdf5r", "httr", "httr2", "htmltools", "jsonlite", "kohonen", 
  "leaflet.providers", "leafem", "libgeos","luz","lwgeom", "leaflet", "leafgl",
  "mapedit", "mapview", "maptiles", "methods", "mgcv", "ncdf4", "nnet", "openxlsx", 
  "parallel", "plotly", "randomForest", "rasterVis", "raster", "Rcpp", "RcppArmadillo", 
  "RcppCensSpatial", "rayshader", "RcppEigen", "RcppParallel", "RColorBrewer", "reactable", 
  "rgl", "rsconnect","RStoolbox", "rts", "s2", "sf", "scales", "sits", "sitsdata", "spdep", 
  "stars", "stringr","supercells", "terra", "testthat", "tidyverse", "tidyterra","tools", 
  "tmap", "tmaptools", "terrainr", "xgboost", "webshot", "webshot2",
  prompt = F)
...
## 3.1  Time Series Data Cubes  {.unnumbered}

Data cubes provide a structured framework for organizing and analyzing satellite imagery time
series. We define data cubes as multidimensional arrays with dimensions associated with space and
time, where land cover observations become "things" that have duration along the temporal dimension.

For REDD+ monitoring, the canonical data cube structure consists of:
...
library(sits)
library(sitsdata)
tempdir_r <- "~/repos/sitsbook/tempdir/R/ts_basics"
...
```{r}
#| eval: false
#| label: datacube-structure
#| code-summary: "Data cube structure for land cover time series"
...
### Temporal and Spatial Sampling  {.unnumbered}

The data cube architecture directly influences activity data accuracy through sampling decisions
along temporal and spatial dimensions. Key considerations include:
...
-   **Temporal compositing**: Median vs. mean aggregation affects cloud/shadow handling
-   **Gap filling**: Missing observations require interpolation, introducing uncertainty

**Spatial sampling effects:**

-   **Pixel resolution**: Trade-off between coverage and computational burden
-   **Spatial aggregation**: Resampling methods (nearest neighbor, bilinear, cubic) affect edge
    accuracy
...
#| code-summary: "Quantifying sampling-induced uncertainty"

# Function to assess temporal sampling impact
assess_temporal_sampling <- function(cube, freq_months = c(12, 24, 36)) {
  ...
  # Reference: annual observations
  ref_change <- detect_change(cube, dt = "P1Y")
  ref_area <- sum(ref_change > 0, na.rm = TRUE)
  
  for (i in seq_along(freq_months)) {
    # Resample to different frequency
    test_cube <- aggregate_time(cube, 
                                 dt = paste0("P", freq_months[i], "M"))
    test_change <- detect_change(test_cube)
    test_area <- sum(test_change > 0, na.rm = TRUE)
    
    results$n_observations[i] <- dim(test_cube)[3]
    results$change_detected[i] <- test_area
    results$omission_error[i] <- (ref_area - test_area) / ref_area * 100
  }
...
# Run assessment
sampling_impact <- assess_temporal_sampling(lulc_cube)
print(sampling_impact)
```

### Array Operations for Land Cover Analysis {#sec-array-operations}

Data cube operations enable efficient land cover change analysis through dimensional manipulation:

**Filter operations** (subset along dimensions):

```{r}
#| eval: false
# Extract specific time period
pre_period <- filter_time(lulc_cube, "2015-01-01", "2019-12-31")
post_period <- filter_time(lulc_cube, "2020-01-01", "2024-12-31")

# Extract specific spatial extent
roi_cube <- filter_geom(lulc_cube, st_read("aoi_boundary.shp"))
```

**Apply operations** (pixel-wise functions):

```{r}
#| eval: false
# Calculate NDVI time series
ndvi_cube <- apply_pixel(lulc_cube, function(x) {
  nir <- x["B5"]
  red <- x["B4"]
  (nir - red) / (nir + red)
}, names = "NDVI")
...
**Reduce operations** (aggregation across dimensions):

```{r}
#| eval: false
# Annual maximum NDVI
ndvi_annual_max <- reduce_time(ndvi_cube, "max(NDVI)")

# Mean NDVI per land cover class
ndvi_by_class <- reduce_space(ndvi_cube, 
                               FUN = mean,
                               zone = lulc_classification)
```

**Aggregate operations** (resampling to coarser resolution):

```{r}
#| eval: false
# Aggregate to 1km resolution for regional analysis
regional_cube <- aggregate_space(lulc_cube, dx = 1000, dy = 1000)

# Multi-year composites
decadal_cube <- aggregate_time(lulc_cube, dt = "P10Y")
```

### Propagation of Uncertainty Through Cube Operations {#sec-cube-uncertainty}

Each data cube operation introduces or propagates uncertainty that must be quantified:

```{r}
#| eval: false
#| label: cube-uncertainty-propagation
#| code-summary: "Uncertainty propagation in data cube workflows"

# Function to track uncertainty through cube operations
propagate_cube_uncertainty <- function(cube, 
                                       classification_cv = 0.05,
                                       registration_error_m = 30,
                                       temporal_lag_months = 6) {
  
  # Calculate base uncertainties
  pixel_size <- cube_view$dx
  n_timesteps <- dim(cube)[3]
  
  # Spatial uncertainty from registration
  spatial_cv <- registration_error_m / pixel_size
  
  # Temporal uncertainty from observation lag
  temporal_cv <- 0.02 * (temporal_lag_months / 12)
  
  # Combined uncertainty (assuming independence)
  total_cv <- sqrt(classification_cv^2 + spatial_cv^2 + temporal_cv^2)
  
  # Propagate through aggregation (uncertainty scales with sqrt(n))
  aggregation_factor <- (cube_view$dx * cube_view$dy) / 
                        (regional_cube$dx * regional_cube$dy)
  aggregated_cv <- total_cv / sqrt(aggregation_factor)
  
  results <- data.frame(
    source = c("Classification", "Registration", "Temporal lag", 
               "Combined", "After aggregation"),
    cv_percent = c(classification_cv, spatial_cv, temporal_cv, 
                   total_cv, aggregated_cv) * 100
  )
...
uncertainty_budget <- propagate_cube_uncertainty(
  lulc_cube,
  classification_cv = 0.05,
  registration_error_m = 30,
  temporal_lag_months = 6
...
### Activity Data Extraction from Cubes {#sec-ad-extraction}

Final activity data for REDD+ accounting requires extracting area estimates with associated
uncertainty from the classified data cube:

```{r}
#| eval: false
#| label: activity-data-extraction
#| code-summary: "Extract activity data with uncertainty"

# Function to extract activity data
extract_activity_data <- function(classified_cube, 
                                  uncertainty_cube,
                                  classes = c("Forest", "Non-forest")) {
  
  results <- list()
  
  for (year in seq(2015, 2024)) {
    # Extract single year
    year_cube <- filter_time(classified_cube, 
                             paste0(year, "-01-01"),
                             paste0(year, "-12-31"))
    
    year_unc <- filter_time(uncertainty_cube,
                           paste0(year, "-01-01"),
                           paste0(year, "-12-31"))
    
    # Calculate areas
    areas <- zonal_statistics(year_cube, 
                              FUN = function(x) {
                                table(x) * prod(cube_view$dx, cube_view$dy) / 10000
                              })
    
    # Calculate uncertainty
    uncertainty <- zonal_statistics(year_unc,
                                   FUN = function(x) {
                                     mean(x, na.rm = TRUE)
                                   })
    ...
    results[[as.character(year)]] <- data.frame(
      year = year,
      class = names(areas),
      area_ha = as.numeric(areas),
      uncertainty_cv = uncertainty
    )
  }
...
# Extract final activity data
ad_results <- extract_activity_data(
  classified_cube = lulc_cube,
  uncertainty_cube = uncertainty_cube
)

# Calculate change statistics
ad_change <- ad_results %>%
  group_by(class) %>%
  arrange(year) %>%
  mutate(
    change_ha = area_ha - lag(area_ha),
    change_percent = (change_ha / lag(area_ha)) * 100
  )

print(ad_change)
```

The data cube framework provides:

1.  Consistent spatiotemporal reference system
2.  Efficient memory management for large datasets
3.  Reproducible analysis workflows
4.  Clear uncertainty propagation pathways
5.  Direct integration with classification and validation workflows

----------------------------------------------------------------------------------------------------

## 3.2 IPCC Land Monitoring Approaches {#sec-ipcc-approaches}

The architecture of land-use transition monitoring systems determines the complexity and uncertainty
of activity data. IPCC guidelines define three hierarchical approaches that differ in spatial
explicitness and data requirements.

### Approach Classification {#sec-approach-classification}

| Approach   | Data Requirement        | Spatial Explicitness         | Uncertainty |
|------------|-------------------------|------------------------------|-------------|
| Approach 1 | Total areas by category | None (aggregated statistics) | Highest     |
| Approach 2 | Transition matrices     | Sampling-based               | Moderate    |
| Approach 3 | Wall-to-wall maps       | Spatially explicit           | Lowest      |

**REDD+ Standard**: Most carbon projects use Approach 3 with remote sensing classification,
combining high spatial resolution with complete coverage. This approach enables:

-   Pixel-level change detection
-   Spatial stratification for uncertainty analysis
-   Integration with degradation monitoring
-   Transparent, verifiable methodologies

### Approach 1: Aggregated Statistics {#sec-approach-1}

Approach 1 estimates total land areas by category without spatial information or transition
tracking.

**Methodology:** - Inventory surveys or statistical sampling - Annual land use totals by category -
Net change calculation (no gross transitions)

**Uncertainty sources:** - Sample design inadequacy - Non-response bias in surveys - Definition
inconsistencies across time - Lack of spatial validation

**Not suitable for REDD+** due to inability to: - Locate deforestation events - Verify permanence -
Support stratified sampling - Enable independent validation

### Approach 2: Sampling-Based Transitions {#sec-approach-2}

Approach 2 uses systematic sampling to estimate land-use transition matrices without complete
spatial coverage.
...
**Advantages:** - Lower data requirements than wall-to-wall mapping - Statistical foundation for
uncertainty - Can incorporate field validation directly

**Limitations for REDD+:** - Cannot produce spatially explicit maps - Difficult to stratify by
drivers or risk - Limited ability to detect small/dispersed changes - Sample size requirements may
be prohibitive for rare transitions

### Approach 3: Spatially Explicit Mapping {#sec-approach-3}

Approach 3 provides complete spatial coverage through wall-to-wall classification of satellite
imagery.
...
**Advantages for REDD+:** - Complete spatial coverage - Enables stratification and targeting -
Supports independent validation - Transparent and reproducible - Compatible with degradation
monitoring

**Uncertainty sources:** - Classification errors (commission/omission) - Geometric co-registration -
Temporal inconsistencies - Edge effects and mixed pixels
...
## 3.3 Pixel-Level Uncertainty Tracking {#sec-pixel-uncertainty}

### Model Residuals as Uncertainty Metrics {#sec-model-residuals}

Random Forest and other machine learning classifiers produce pixel-level probability estimates that
can be converted to uncertainty metrics. These residuals identify areas requiring additional
training data or validation.

**Theoretical foundation:**

For each pixel $i$, Random Forest produces class probabilities $\{p_{i,1}, p_{i,2}, ..., p_{i,k}\}$
across $k$ classes. Uncertainty metrics include:

$$
\text{Entropy}_i = -\sum_{j=1}^{k} p_{i,j} \log(p_{i,j})
$$

$$
\text{Margin}_i = p_{i,\text{max}} - p_{i,\text{second}}
$$

$$
\text{Prediction strength}_i = \max(p_{i,j})
$$
...
```{r}
#| eval: false
#| label: pixel-uncertainty-metrics
#| code-summary: "Calculate pixel-level uncertainty from RF probabilities"
...
calculate_pixel_uncertainty <- function(rf_model, imagery_stack) {
  ...
  # 3. Maximum probability (prediction strength)
...
The pixel-level uncertainty framework enables: - Adaptive sampling design - Efficient resource
allocation - Continuous model improvement - Spatially explicit confidence intervals
...
Monte Carlo cross-validation (MCCV) provides robust assessment of Random Forest model performance by
repeatedly partitioning training data into calibration and validation sets.

**Objectives:** 1. Optimize hyperparameters (ntree, mtry, nodesize) 2. Assess model stability across
random partitions 3. Quantify prediction uncertainty 4. Select optimal training sample size

**Theoretical framework:**

For $M$ Monte Carlo iterations: 1. Randomly partition training data: 70-80% calibration, 20-30%
validation 2. Train Random Forest model on calibration subset 3. Evaluate on validation subset 4.
Record performance metrics (OOB error, accuracy, kappa) 5. Aggregate results across iterations
...
Monte Carlo cross-validation provides: 1. Robust performance estimates 2. Hyperparameter
optimization 3. Sample size guidance 4. Model stability assessment 5. Uncertainty quantification for
predictions
...
**Producer's Accuracy** (Sensitivity, Recall): - Probability that reference class is correctly
classified - $PA_i = \frac{n_{ii}}{\sum_j n_{ij}}$ where $n_{ii}$ is diagonal, $\sum_j n_{ij}$ is
column total - Measures omission error: $OE_i = 1 - PA_i$ - Critical for REDD+: Low forest
producer's accuracy → missed deforestation

**User's Accuracy** (Precision, Positive Predictive Value): - Probability that classified pixel
truly belongs to that class\
- $UA_i = \frac{n_{ii}}{\sum_j n_{ji}}$ where $\sum_j n_{ji}$ is row total - Measures commission
error: $CE_i = 1 - UA_i$ - Critical for REDD+: Low forest user's accuracy → over-credited forest
area

**Overall Accuracy**: - Proportion of correctly classified pixels -
$OA = \frac{\sum_i n_{ii}}{n_{total}}$ - Simple but can be misleading with imbalanced classes

**Kappa Coefficient**: - Agreement beyond chance - $\kappa = \frac{OA - P_e}{1 - P_e}$ where $P_e$
is expected agreement by chance - Interpretation: \<0.4 (poor), 0.4-0.6 (moderate), 0.6-0.8
(substantial), \>0.8 (excellent)

**F1 Score**: - Harmonic mean of precision and recall -
$F1_i = 2 \times \frac{PA_i \times UA_i}{PA_i + UA_i}$ - Balances omission and commission errors

### Area-Adjusted Accuracy Metrics {#sec-area-adjusted-accuracy}

Standard confusion matrices from validation samples must be adjusted to account for sampling design
...
## 3.7 Chapter Summary {#sec-summary}

This chapter established a comprehensive framework for activity data uncertainty quantification in
REDD+ monitoring systems. Key contributions include:
...
Activity data provides spatial extent; allometry converts extent to biomass. Combined uncertainty
requires:
...
Where covariance term captures spatial correlation between classification errors and biomass
estimation errors.
