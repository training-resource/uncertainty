...
For REDD+ carbon accounting, we tend to focus exclusively on ontogenic allometry to estimate relationships between accessible tree dimensions and total aboveground biomass as trees grow from seedling to maturity.
...
::: callout-tip
In specialized growing environments, such as mangrove habitats, the choice of allometry weighs heavily on emissions accuracy. For example, @rocha2018reducing found generic allometries to produce significantly wider error margins in biomass estimations than species-specific equations (-18% & +14%) across Brazilian mangrove forests. To compensate, generic equations incorporate proxies of environmental stress, such as wood density, height-diameter ratios or "stunting", and species composition.
...
We filter `scbi_stem1` to *Quercus* observations, the largest genus subsample available, to compare the impact of scaling allometry on uncertainty. Missing entries are also removed, providing a new sample of 84 stems from the dataset's total measurements of 2,287 trees.
...
2.  Species-specific equations: Reduces RMSE by 3-4 percentage points vs. genus-level, protecting \$10-15k per million tCO₂e
...
5.  Sample size adequacy: Minimum n≥50 trees per equation (Roxburgh et al. 2015), with stratification across ln(DBH) range
