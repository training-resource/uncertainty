...
## Allometric Equations {#sec-allometry-relationship}
...
# check distribution 
psych::describe(scbi_quercus) |> dplyr::select(!c(trimmed,trimmed, mad, range)) |> 
...
::: callout-tip
At this early stage in the workflow, it helps to quickly check the range and spread of DBH values. These values offer early indicators of the statistical operations and validations needed ahead.
:::
...
The following functions from the `BIOMASS` package [@BIOMASS] require some additional wrangling, specifically to extract values from the Global Wood Density database [@vieilledent2018new] and convert from units provided [@rejou2017biomass].

::: callout-tip
It is recommended to check if selected equations require any unit conversions or scaling limiters. For example, if using the most common allometry from Chave et al (2014), we need to convert from milligrams to kilograms, as follows.
:::
