...
easypackages::packages(
  ...
  "htmltools",
  "janitor", "jsonlite", 
  "lattice", "leaflet.providers", "leafem", "lwgeom", "leaflet", "lmtest",
  "kableExtra", "kernlab", "knitr", 
  "mapedit", "mapview", "maptiles", "Mlmetrics", "moments",
  "olsrr", "openxlsx",
  "plotly", "psych", 
  "randomForest", "raster","RColorBrewer", "rmarkdown", "renv",
...
model selection, parameter estimation, and field
measurements. Log-transformation adds complexity through
back-transformation bias that must be corrected to avoid
systematically miscalulcating biomass.
...
```{r}
#| eval: false
# load allometry database package
library("allodb")

# import allomtry dataset from allodb
data(scbi_stem1)
```
...
::: callout-tip
At this early stage, it is worth taking a quick note of the
range and spread in DBH values spanning from small
understory cohorts to canopy dominants exceeding 100 cm
diameter. This offers an early indicator of statistical
operations and validation checks that may be needed ahead.
:::
...
```{r}
# visualize all available variables
...
::: callout-tip
Traditional Null Hypothesis testing is susceptible to Type
II and III errors , where insufficient statistical power
from small sample size leads to incorrectly accepting models
with unacceptable differences[@paul2018validation]. The
following validation metrics are highlighted in guidelines
provided by the NASA Working Group on Land Product
Calibration and Validation
[@duncansonAbovegroundWoodyBiomass2021] :

-   Statistical Power: Minimum sample size considered to
    ensure statistical power, targeting \>50 per class or
    species.
-   Equivalence Margin: Acceptable parity, often set as a
    percentage of equivalence threshold (\<25% RMSE~*rel*~)
-   Data Structures: Validation data exhibits similar
    dimensions as calibration data, such as tree-size
    structure and range, particularly among largest cohorts
    which hold disproportionate share of biomass variable.
:::

## Normality Testing

on-normal distributions violate assumptions of parametric
...
misspecifies the functional form.
...
cat(sprintf("Linear Model RMSE: %.1f kg (%.0f%%)\n", rmse_linear, rel_rmse_linear))
cat(sprintf("Log-Transformed Model RMSE: %.2f on log-scale (%.1f%% relative)\n", 
            rmse_log, rel_rmse_log))
cat(sprintf("\nUncertainty reduction: %.0f percentage points\n", 
            rel_rmse_linear - rel_rmse_log))
```
...
cat(sprintf("MAE: %.2f kg\n", mae_cv))
cat(sprintf("R²: %.4f\n", r2_cv))
cat(strrep("=", 50), "\n", sep = "")

# Visualize
ggplot(cv_predictions, aes(x = pred, y = obs)) +
  geom_point(alpha = 0.3, shape = 1) + 
  geom_abline(slope = 1, intercept = 0, color = 'blue', linewidth = 1) +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  coord_fixed() + labs(title = "Cross-Validated Predictions", subtitle = sprintf(
  	"RMSE = %.1f kg | R² = %.3f | n = %d", rmse_cv, r2_cv, nrow(cv_predictions)),
  	x = "Predicted AGB (kg)", y = "Observed AGB (kg)") + theme_minimal()
...
# ART-TREES classification
tier <- ifelse(uncertainty_pct <= 10, "Tier 3 (≤10%)",
               ifelse(uncertainty_pct <= 20, "Tier 2 (10-20%)", "Tier 1 (>20%)"))

# Calculate approximate UA factor (ART-TREES Equation 11)
