...
The study of allometric relationships in the scaling of morphological features relate to three biological phenomena:

-   Ontogenic allometry: Size-related changes within an individual organism's growth trajectory. This is the primary focus of tree biomass estimation, where DBH-biomass relationships capture how trees allocate resources as they mature [@chave2005a; @brown1997a]
-   Static allometry: Size variation among individuals of the same age or developmental stage, reflecting genetic or environmental differences within a population [@niklas1994plant]
-   Evolutionary allometry: Phylogenetic variation in size-scaling relationships across species or lineages, representing adaptive divergence in morphological strategies [@russo2007growth]

For REDD+ carbon accounting, we focus exclusively on ontogenic allometry to target the predictable relationship between accessible tree dimensions and total aboveground biomass as trees grow from seedling to maturity.

::: callout-important
Species within a genus share evolutionary history but may differ in wood density, branch architecture, or bole form due to adaptation to local conditions. For example:
...
Allometric equations are classified by taxonomic scope and environmental specificity, representing a trade-off between precision and generalization:
...
::: callout-tip
In specialized growing environments, such as mangroves, the choice of allometry weighs heavily on emissions accuracy. For example, @rocha2018reducing found generic allometries to produce significantly wider error margins in biomass estimations than species-specific equations (-18% & +14%) across Brazilian mangrove forests. To compensate, generic equations incorporate proxies of environmental stress, such as wood density, height-diameter ratios or "stunting", and species composition.

However, while inclusion of tree height significantly reduces bias in AGB estimates [@rutishauser2013generic; @chave2014improved], the accurate measurement of tree height in closed-canopy forests is especially challenging[@king2011allometry]. In addition, due to resulting variance in height data with plots and at different spatial scales, height-diameter (H-D) models tend to exhibit high uncertainty [@feldpausch2011height; @valbuena2016sensitivity]. LiDAR and RADAR technologies can improve height measurement accuracy but are not always available for REDD+ project areas.
:::

In practice, the choice of equation category involves balancing precision, cost, and data availability:

| Category | Precision | Data Requirements | Geographic Applicability | Recommended Use |
|---------------|---------------|---------------|---------------|---------------|
| Species-specific | Highest (±10-15%) | Species ID, local calibration | Narrow (same region/climate) | High-value carbon projects, limited species diversity |
| Genus-specific | Moderate (±15-20%) | Genus ID | Moderate (similar forest types) | Mixed-species temperate forests |
| Pan | Lower (±20-30%) | DBH, WD, sometimes H | Broad (tropical/temperate zones) | Data-limited contexts, preliminary assessments |
| Environmentally-conditioned | Highest\* (±12-18%) | DBH, WD, H, environmental variables | Moderate (similar environmental conditions) | Specialized ecosystems (mangroves, peatlands), jurisdictional REDD+ |

: \**When environmental variables are properly measured and equation applied within calibration range*
...
scbi_stem1 |> dplyr::group_by(Family, genus, species) |>
	dplyr::summarise(`Total trees sampled` = n()) |>
	flextable::flextable()|> flextable::autofit()
```
...
We filter `scbi_stem1` to *Quercus* observations, which is the largest genus subsample available, removing outliers and missing entries to retain trees with DBH 5-100 cm. This produces a new sample of 71 stems from the dataset's total measurements of 2,287 trees.
...
::: callout-tip
At this early stage in the workflow, it helps to quickly scan the range and spread of DBH values. These values offer early indicators of the statistical operations and validations needed ahead. For example, notice the DBH range from 5.4-83.5 cm, which will inform equation selection criteria directly below.
:::
...
3.  DBH Range Coverage: Ensure equation applicability spans ≥80% of measured diameter distribution
...
*Table X: List of variables in the `equations` dataset available to use as selection criteria in allometry development using `allodb` package.*
...
#  Added filtering and variable display for HTML renders   #
...
::: callout-tip
## Database Navigation Note

In the following code chunk, we filter equations geographically with the `allodb` latitude and longitude variables. This is done using `dplyr` functions instead of the suggested filter `new_equation()` due to inconsistencies found in the `geographic_area` field.

In subsequent cells, we import pan-tropical equations and fit them with values from by Global Wood Density database using the `computeAGB()` and`getWoodDensity()` functions from the `BIOMASS` package.
:::
...
eq_region_genus_dbh <- eq_region_genus |>
  ...
  dplyr::filter(!is.na(dbh_min_cm), !is.na(dbh_max_cm),
    dbh_min_cm <= field_dbh_min * 15, # extrapolation allowed for saplings
...
eq_region_species_dbh <- eq_region_species |>
  dplyr::mutate(
  	...
  	dbh_max_cm = as.numeric(dbh_max_cm)) |>
  dplyr::filter(!is.na(dbh_min_cm), !is.na(dbh_max_cm),
    dbh_max_cm >= field_dbh_max * 0.6) # extrapolation allowed for crown trees 
...
eq_region_genus_dbh_sample <- eq_region_genus_dbh |>
  dplyr::filter(sample_size >= 50)  # Minimum for genus-specific equations

eq_region_species_dbh_sample <- eq_region_species_dbh |>
  dplyr::filter(sample_size >= 50)  # Minimum for species-specific equations
...
equation_metadata <- data.frame(
  ...
  Threshold = c(
    ...
    "n≥50 (Roxburgh et al. 2015: CV~7.5%)"
  ),
  Selected_Value = c(
  ...
  Justification = c(
    ...
    "Exceeds threshold for CV=7.5% precision"
  )
)
...
::: callout-tip
## Allometry Selection Best Practices

1.  Transparency: Document all criteria and thresholds
2.  Reproducibility: Code-based workflow enables auditing
3.  Bias Reduction: Geographic and taxonomic filtering minimizes systematic errors
4.  Uncertainty Quantification: Multiple equations enable sensitivity analysis (Section 1.5)

For REDD+ MRV reporting under ART-TREES/VCS, include selection rationale demonstrating: - Compliance with equation selection requirements - Justification for any extrapolation beyond equation DBH ranges - Geographic/climatic appropriateness - Sample size adequacy for target precision
:::
...
Having identified candidate allometry equations, we can apply these to calculate aboveground biomass estimates using three approaches: species-specific, genus-level, and pan-tropical (`BIOMASS`).

::: callout-tip
## Calling Generic Models

Species-specific equations are automatically fitted when using default `allodb::get_biomass()` function. The `BIOMASS` package provides globally-applicable equations that require wood density extraction from the Global Wood Density Database. These tools are useful resources, including a number of other allometric packages specific to different biomes like `silviculture`, `forestdata`, `ForestBiomass`, and the recommended option `rFVS.`
:::
...
lin_species_mae			= ModelMetrics::mae(scbi_quercus$agb_species, lin_species_resid) |> round(2)
lin_species_rmse		= ModelMetrics::rmse(scbi_quercus$agb_species, lin_species_resid) |> round(2)
lin_species_rmse_rel= round(lin_species_rmse / mean(scbi_quercus$agb_species, na.rm = T) * 100,2)
log_species_mae 		= ModelMetrics::mae(scbi_quercus$agb_species, log_species_resid) |> round(2)
log_species_rmse		= ModelMetrics::rmse(scbi_quercus$agb_species, log_species_resid) |> round(2)
log_species_rmse_rel= round(log_species_rmse / mean(scbi_quercus$agb_species, na.rm = T) * 100,2)
# ******** CRITICAL back-transformation = log_species_rmse_rel

# Genus-level
lin_genus_mae 			= ModelMetrics::mae(scbi_quercus$agb_genus, lin_genus_resid) |> round(2)
lin_genus_rmse 			= ModelMetrics::rmse(scbi_quercus$agb_genus, lin_genus_resid) |> round(2)
lin_genus_rmse_rel	= round(lin_genus_rmse / mean(scbi_quercus$agb_genus, na.rm=T) * 100,2)
log_genus_mae 			= ModelMetrics::mae(scbi_quercus$agb_genus, log_genus_resid) |> round(2)
log_genus_rmse			= ModelMetrics::rmse(scbi_quercus$agb_genus, log_genus_resid) |> round(2)
log_genus_rmse_rel	= round(log_genus_rmse / mean(scbi_quercus$agb_genus, na.rm=T) * 100, 2)
# ******** CRITICAL back-transformation = log_genus_rmse_rel ***********

# Chave 2014
lin_chave2014_mae			= ModelMetrics::mae(scbi_quercus$agb_chave2014, lin_chave2014_resid) |> round(2)
lin_chave2014_rmse		= ModelMetrics::rmse(scbi_quercus$agb_chave2014, lin_chave2014_resid) |> round(2)
lin_chave2014_rmse_rel= round(lin_chave2014_rmse /mean(scbi_quercus$agb_chave2014,na.rm=T) * 100, 2)
log_chave2014_mae			= ModelMetrics::mae(scbi_quercus$agb_chave2014, log_chave2014_resid) |> round(2)
log_chave2014_rmse		= ModelMetrics::rmse(scbi_quercus$agb_chave2014, log_chave2014_resid) |> round(2)
log_chave2014_rmse_rel= round(log_chave2014_rmse / mean(scbi_quercus$agb_chave2014, na.rm=T) * 100, 2)

# Brown 1997
lin_brown1997_mae			= ModelMetrics::mae(scbi_quercus$agb_brown1997, lin_brown1997_resid) |> round(2)
lin_brown1997_rmse 		= ModelMetrics::rmse(scbi_quercus$agb_brown1997, lin_brown1997_resid) |> round(2)
lin_brown1997_rmse_rel= round(lin_brown1997_rmse / mean(scbi_quercus$agb_brown1997, na.rm=T) * 100, 2)
...
# Chave 2005
linear_chave2005_mae			= ModelMetrics::mae(scbi_quercus$agb_chave2005, lin_chave2005_resid) |> round(2)
linear_chave2005_rmse			= ModelMetrics::rmse(scbi_quercus$agb_chave2005, lin_chave2005_resid) |> round(2)
linear_chave2005_rmse_rel = (linear_chave2005_rmse / mean(scbi_quercus$agb_chave2005, na.rm=T) * 100)|> round(2)
log_chave2005_mae 				= ModelMetrics::mae(scbi_quercus$agb_chave2005, log_chave2005_resid) |> round(2)
log_chave2005_rmse				= ModelMetrics::rmse(scbi_quercus$agb_chave2005, log_chave2005_resid) |> round(2)
log_chave2005_rmse_rel		= (log_chave2005_rmse / mean(scbi_quercus$agb_chave2005, na.rm=T) * 100) |> round(2)
...
split_verification |>
  flextable::flextable() |>
  flextable::set_caption("Calibration and Validation Set Distribution - Proportionality Check") |>
...
mc_performance |>
  ...
  flextable::set_header_labels(
    ...
    MAE = "MAE (kg)",
    RMSE = "RMSE",
    Rel_RMSE_pct = "Rel. RMSE (%)",
    R2 = "R²",
    Credit_Deduction_pct = "Credit Deduction (%)"
  ) |>
  flextable::align(j = 1:2, align = "left", part = "all") |>
  flextable::align(j = 3:9, align = "right", part = "all") |>
  flextable::fontsize(size = 8, part = "all") |>
  flextable::autofit() |>
  flextable::add_footer_lines(
    "Monte Carlo LGOCV with 100 iterations (80/20 split). Shapiro p-value tests normality of residuals. Credit deduction calculated per ART-TREES methodology. Red highlighting indicates values exceeding 10% threshold."
  ) |>
...
# Enhanced table showing proportionality preservation
split_verification |>
  flextable::flextable() |>
  flextable::set_caption("Calibration and Validation Set Distribution - Proportionality Check") |>
  flextable::set_header_labels(
  ...
  flextable::colformat_double(j = c("cal_pct", "val_pct", "Difference_pct"), digits = 1) |>
  flextable::fontsize(size = 9, part = "all") |>
  flextable::bold(j = c("cal_pct", "val_pct"), part = "body") |>
  flextable::bg(j = "cal_pct", bg = "#e8f4f8", part = "body") |>
  flextable::bg(j = "val_pct", bg = "#fff4e6", part = "body") |>
  flextable::bg(i = ~ Difference_pct > 2, j = "Difference_pct", bg = "#ffcccc") |>
  flextable::autofit() |>
  flextable::add_footer_lines(
    "Stratified sampling ensures DBH class proportions remain similar between datasets. 'Cal. % of Total' and 'Val. % of Total' show each class as % of its respective dataset. Δ shows absolute difference - values <2% indicate good proportionality preservation."
  ) |>
