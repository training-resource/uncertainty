...
## The Allometric Relationship {#sec-allometry-relationship}
...
AGB: Aboveground biomass (kg) - DBH: Diameter at breast
height (cm) - α, β: Allometric parameters
(species/biome-specific) - ε: Random error term

### Sources of Uncertainty {#sec-allometry-sources}

1\. Model selection uncertainty: - Geographic
transferability (tropical vs. temperate) - Species-specific
vs. generic equations - Functional form (power law vs.
polynomial)

2\. Parameter estimation uncertainty: - Regression
coefficient standard errors - Sample size of calibration
dataset - Variance-covariance structure

3\. Measurement uncertainty: - DBH measurement precision
(±0.5-1.0 cm) - Height measurement bias (clinometer error) -
Wood density sampling variability

4\. Bias correction requirements: - Log-transformation
back-transformation bias - Non-linear model prediction
intervals - Censored data and zero-inflation

### ART Allometry Exemption {#sec-allometry-exemption}

Critical policy: ART-TREES V2.0 Section 8 states:

> "Structural uncertainty in allometric models is excluded
> from uncertainty deductions when models are applied
> consistently between baseline and crediting periods."

Rationale: - Systematic bias cancels in net change
calculations - Reduces burden on project developers -
Focuses uncertainty on precision (random error), not
accuracy (systematic bias)

What must still be reported: - Parameter estimation
uncertainty (confidence intervals) - Measurement precision
(DBH, height, wood density) - Sampling uncertainty
(plot-to-landscape scaling) - Model structural uncertainty
(exempted for consistent application)

Strategic implication: Projects can reduce uncertainty
deductions by: 1. Demonstrating consistent model application
2. Upgrading from Tier 1 (IPCC defaults) to Tier 2
(country-specific) 3. Investing in measurement precision
rather than destructive sampling

------------------------------------------------------------

## Model Selection {#sec-model-selection}

### Available Allometric Equations {#sec-available-equations}

Example dataset: Four species in UK temperate forest
...
# Database of allometric equations
allometric_equations <- tribble(
  ~Species, ~Model_ID, ~Form, ~Alpha, ~Beta, ~DBH_Range_cm, ~Country, ~Reference,
  
  # Betula pendula (Silver Birch)
  "Betula pendula", "M1", "α×D^β", 0.25110, 2.29000, "All", "UK", "Bunce (1968)",
  "Betula pendula", "M2", "exp(α+β×ln(D))", -2.41660, 2.42270, "2.9-30", "UK", "Bunce (1968)",
  "Betula pendula", "M3", "exp(α+β×ln(D))", -2.75840, 2.61340, "2.9-26", "UK", "Bunce (1968)",
  "Betula pendula", "M4", "exp(α+β×ln(D))", -2.16250, 2.30780, "3.3-16", "UK", "Hamilton (1975)",
  "Betula pendula", "M5", "exp(α+β×ln(D))", -2.64230, 2.46780, "3.5-23", "UK", "Hamilton (1975)",
  
  # Betula pubescens (Downy Birch)
  "Betula pubescens", "M6", "exp(α+β×ln(D))", -2.16200, 2.30780, "10-90", "UK", "Matthews (1987)",
  "Betula pubescens", "M7", "α×D^β", 0.00029, 2.50038, "0.8-8.5", "Sweden", "Johansson (1999)",
  
  # Picea sitchensis (Sitka Spruce)
  "Picea sitchensis", "M8", "exp(α+β×ln(D))", -3.03000, 2.55670, "3-283", "USA", "Gholz (1979)",
  "Picea sitchensis", "M9", "α×D^β", 0.02800, 2.71000, "2-40", "Ireland", "Black (2004)",
  
  # Sorbus aucuparia (Rowan)
  "Sorbus aucuparia", "M10", "exp(α+β×ln(D))", -2.21180, 2.41330, "3-64", "USA", "Jenkins (2003)",
  "Sorbus aucuparia", "M11", "exp(α+β×ln(D))", -2.92550, 2.41090, "3-60", "USA", "Jenkins (2003)"
)

kable(
  allometric_equations,
  caption = "Available Allometric Equations by Species",
  col.names = c("Species", "Model", "Form", "α", "β", "DBH Range (cm)", "Country", "Reference")
)
```
